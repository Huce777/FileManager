...
def check_file_consistency(file_path):
    ...
    if actual_type == file_extension:
        print(f"文件{file_path}的后缀与内容一致")
        return True, actual_type
    else:
        print(f"文件{file_path}的后缀与内容不一致，实际类型为{actual_type}")
...
程序说明
    功能概述
这个程序通过文件头识别文件的实际类型，并检查文件的后缀名是否与内容一致。它支持您列出的所有文件类型，包括文本文件、图片文件、音频文件、视频文件、压缩文件、可执行文件、表格文件、演示文稿文件、数据库文件、网页文件、代码文件和配置文件。
    使用方法
将上述代码保存为 file_consistency_checker.py
    在命令行运行以下命令:
    python file_consistency_checker.py [目录路径]
    例如:
    python file_consistency_checker.py C:/Users/YourName/Documents

程序结构
    FILE_SIGNATURES: 定义了常见文件类型的文件头签名
    get_file_type(): 通过文件头获取文件的实际类型
    check_file_consistency(): 检查文件后缀与内容是否一致
    scan_directory(): 扫描目录下的所有文件
    main(): 主函数，处理命令行参数并运行程序

注意事项
    该程序仅支持在代码中定义的文件类型
    对于某些文件类型（如TXT），文件头可能不固定，识别可能不准确
    对于大型文件夹，扫描可能需要较长时间
    程序需要Python 3.6或更高版本运行
"""
"""
